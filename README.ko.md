...
- 감사가 공식 도구가 대상으로 하는 결함을 발견하면, 답변에서 그것을 선택적
  다음 단계로 안내합니다(실행은 직접): CLAUDE.md가 취약하면
  `claude-md-management` 플러그인, 코드 인텔리전스가 취약하면 언어별 LSP
  플러그인, 스킬이 없으면 `skill-creator` 플러그인. 감사 자체는 아무것도
  설치하지 않으며 읽기 전용을 유지합니다.
