...
- 監査が公式ツールの対象となるギャップを見つけた場合、返信でそれを任意の
  次のステップとして案内します(実行はご自身で):CLAUDE.md が弱ければ
  `claude-md-management` プラグイン、コードインテリジェンスが弱ければ言語別
  の LSP プラグイン、スキルが無ければ `skill-creator` プラグイン。監査自体は
  何もインストールせず、読み取り専用のままです。
